...
$E$ &::= & $v$ \\
  ...
  %  & | & $[\overline{\tx:\star}]\to T$ & do we want this syntax or remove it and have $\left\{\overline P;\overline{S};\overline{D}\right\}$?\\
    ...
    & | & \textcolor{Gray}{\texttt{behaviour\_of($M$)}} & singleton type of M (modulo path equivalence)
 \\
...
%    & | & $t\wedge t$ & \aghilas{we can remove this one}\\
    ...
    & | & \textcolor{Gray}{\texttt{behaviour\_of($M$)}} &\color{Gray} the singleton type of $M$ (modulo path equivalence)\\[3mm]
...
(\{(d_1^1, \ldots, d_1^m, d'_1),\ldots,(d_n^1, \ldots, d_n^m, d'_n)\}:(t^1,\ldots,t^m)\to t') &= \forall i\in\{1,\ldots,n\}
  ...
  types. Suppose we have a function $\eta$ which associates an atom to an Elixir
  identifier. Then we could represent $\mathcal{X}[\overline{x=t}]$ by the following type:
\[
