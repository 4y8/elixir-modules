...
\beppe{Observation: we need to add to types the case of a type name $\tx$, even if it is not preceded by a path. This is because it can be a type defined previously in the module, or it can be a parameter of the module. In both cases it must be present in $\Gamma$ as $\tx:\star$ to be a well-formed type, and we must encode it as an opaque type. Currently we do not have "empty" paths to obtain this special case}.
...
The rules in Figures~\ref{wf-behenv} state that the empty behaviour environment is well formed; if we assume that type names that are parameters of the behaviour \BX are well formed types, we can deduce the good-formation of $\{\overline{D}\}$, then adding $\BX:\Lambda(\overline{\tx:\star}).\{\overline{D}\}$ to the current behaviour environment yields a well-formed environment. Notice that no behaviour instantiation is given for $\{\overline{D}\}$, since behaviours cannot implement other behaviours.
\begin{figure}[ht]
  \begin{mathpar}
    \inferrule[WFBehEnv-Empty]{\\}{\vdash\epsilon}\hfill
    \inferrule[WFBehEnv-Beh]{\Sigma;\overline{\tx:\star}\vdash\{\overline{D}\}}{\Sigma\vdash\Sigma,\BX:\Lambda(\overline{\tx:\star}).\{\overline{D}\}}
  \end{mathpar}
