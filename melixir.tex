...
$$\tau ::= \texttt{atom} \mid \texttt{mod} \mid \texttt{fun} \mid (\overline{\tau})\to\tau \mid \texttt{\%}\!\left\{\overline{f}\right\} \mid \tau\vee\tau \mid \neg\tau\mid\alpha\mid\mathbb{O}$$
The simple types $t$ are then defined on the top of the core types $\tau$ \emph{inductively} by the following grammar:
$$t ::= \tau \mid  (\overline{t})\to t \mid \texttt{\%}\!\left\{\overline{f:t}\right\} \mid M.\tx \mid \BH M \mid \BX[\overline{\tx=t}]$$
...
and the new types we added to handle modules, which we will call \emph{module types}. The core types are interpreted as sets of values as in \cite{CDV24,CD25}, while the module types are interpreted as singletons modulo the equivalence of their instantiations. This is a bit similar to the stratification of types in \cite{CDV24} into basic types and compound types, where basic types are interpreted as sets of values, while compound types are interpreted as sets of basic types.
