...
\begin{figure}
  \begin{tabular}{r c ll}
    ...
    & | & $\tx = \textsf{opaque }t$                   &\color{Gray} opaque type\\[3mm]
       $P$         & ::= & $X$ | $P.X$                                                             \\[1.5mm]
$M$         & ::= & $X$ | $\Elixir.P[\overline{\tx = t}]$                                   \\[1.5mm]
   $N$ & ::= & $x$ | $X$ \\[1.5mm]
   $E$ &::= & $v$ \\
    & | & $x$ \\
    & | & $\kwlet N = E\kwin E$ \\
    ...
    & | & {\color{OrangeRed}\text{$X$}} \\
    $v$ & ::= & $k$ \\
    & | & $\texttt{\%}\!\left\{\overline{k=v}\right\}$ \\
    & | & $\$\bigwedge \overline{(\,\overline{t}\,)\rightarrow t} \kwfn \overline{x} \rightarrow E$ \\
    & | & {\color{OrangeRed} \text{$\$\bigcap \overline{\left(\overline{N:T}\right)\rightarrow T} \kwfn \overline{N}\rightarrow E$}} &\color{OliveGreen} syntactic intersection\\
    & | & {\color{OrangeRed}\text{$\Elixir.P[\overline{\tx = t}]$}} \\[3mm]
    $T$ & ::= & $t$ \\
    & | & $\left(\overline{N:T}\right)\rightarrow T$ \\
...
%    & | & $t\wedge t$ & \aghilas{we can remove this one}\\
  ...
  %  &&&\color{bordeaux} Important: this allows maps and tuples to contain modules!\\
    ...
    $D$ & ::= & $ X : t$    &\color{Gray} exports a module$X$ whose type is either a singleton or a behaviour or \texttt{mod}\\
   ...
   $\mathcal{D}$ & ::= & $D$ & \\
    & | &  $X : \Lambda(\overline{\tx:\star}).\{\overline{\mathcal{D}}\}_{\overline{\BX[\,\overline{\tx=t}\,]}}$ & \\[3mm]
    $\Gl$ & ::= & $\overline{X : \Lambda(\overline{\tx:\star}).\{\overline{\mathcal{D}}\}_{\overline{\BX[\,\overline{\tx=t}\,]}}}$ &\color{Gray} global type environment\\
 $\Gamma$ & ::= & $\overline{D}$ &\color{Gray} local type environment\\
