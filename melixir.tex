...
\end{minted}
which is thus conforms to the syntax of Figure~\ref{fig:surfacesyntax}, and the type-checker will check the call to \elix{Module.create/3} as any other call, and in particular it will check that the first argument is of type \elix{atom()} (since module names are atoms) and that the second argument is of type \elix{Macro.t()} (since the second argument of \elix{Module.create/3} must be a quoted expression) and that this argument is of type \elix{Macro.Env.t()} (which is the type of the options passed as third argument).
...
\begin{figure}
 ...
 %   & | & $M.M$                                     &\color{Gray} imported module\\
   ...
   $E$ &::= & $v$ \\
    ...
    & | & $\kwlet N = E\kwin E$ \\
    & | & $E(\overline{N})$ \\
    & | & $\texttt{\%}\!\left\{\overline{k=E}\right\}$ & $k$ ranges over keys\\
    & | & $E.k$ \\
    & | & $(E\in t)?E:E$ \\
    ...
    & | & $\textcolor{Cerulean}{(\overline{\tx:\star})\to T}$        & \text{first class parametric modules. Maybe  $\textcolor{Gray}{(\overline{\tx:\star})\to \left\{\overline{D}\right\}_{\overline{\BX}}}$ suffices} \\[3mm] 
...
\item every path $M$ occurring in the pre-type is well-formed, that is, it effectively points to a fully instantiated module;
\end{itemize}
...
Figure~\ref{wf-valenv} defines when a value environment is well-formed. Rule \Rule{WFValEnv-Empty} states that the empty value environment is well-formed for any well-formed behaviour environment. Rule \Rule{WFValEnv-Opaque} states that if $\Gamma$ is well-formed, then adding an opaque type declaration $\tx:\star$ to $\Gamma$ yields a well-formed environment. Rule \Rule{WFValEnv-Trans} requires the good formation of the transparent type $t$. Likewise, rule \Rule{WFValEnv-Value} requires the good formation of the type $T$ of the module $X$ being declared. Again, for the sake of simplicity we examine only the case in which a module definition depends only on the module definitions that precede it, though in Elixir is possible to have mutually recursive module definitions. 

\aghilas{
  We miss a rule for values $x$, we may rename \Rule{WFValEnv-Value} to \Rule{WFValEnv-Module} and add the following rule.
  \begin{mathpar}
    \inferrule[WFValEnv-Value]{\overline{\Sigma;\Gamma\vdash T}}
      {\Sigma;\Gamma\vdash\Gamma, x:\bigcap \overline{T}}
  \end{mathpar}
  In prose, it could be something like: Likewise, rules \Rule{WFValEnv-Module}
  and \Rule{WFValEnv-Value} require the good formation of the types $T$ of the
  ...
  \Rule{WFValEnv-Opaque} doesn't follow the same convention as the others in its
  hypothesis.
...
\begin{figure}[h]
  \begin{mathpar}
    \inferrule[WFValEnv-Empty]{\vdash\Sigma}{\Sigma\vdash\epsilon}\hfill
    \inferrule[WFValEnv-Opaque]{\Sigma\vdash\Gamma}{\Sigma\vdash\Gamma,\tx:\star}\hfill
    \inferrule[WFValEnv-Trans]{\Sigma\vdash\Gamma\\\Sigma;\Gamma\vdash t:\star}{\Sigma\vdash\Gamma,\tx:[= t]}\hfill
    \inferrule[WFValEnv-Value]{\Sigma\vdash\Gamma\\\Sigma;\Gamma\vdash T}{\Sigma\vdash\Gamma,X:T}
    \inferrule[WFValEnv-Value]{\Sigma\vdash\Gamma\\\overline{\Sigma;\Gamma\vdash T}}
...
\begin{figure}[h]
  \begin{mathpar}
    \inferrule[WFValEnv-Empty]{\vdash\Sigma}{\Sigma;\epsilon\vdash\epsilon}\hfill
    \inferrule[WFValEnv-Opaque]{\Sigma\vdash\Gamma}{\Sigma;\Gamma\vdash\Gamma,\tx:\star}\hfill
    \inferrule[WFValEnv-Trans]{\Sigma;\Gamma\vdash t:\star}{\Sigma;\Gamma\vdash\Gamma,\tx:[= t]}\hfill
...
We have to define when a module system type $T$ is well-formed. This is done by the rules in Figure~\ref{wf-modtype}, which essentially check the following conditions: $(1)$ in a function type $(\overline{N:T})\to T'$, the names $N_i$ are  used only in the types $T_j$ for $j>i$ (besides in $T'$); $(2)$ in a module type $\{\overline{D}\}_{\overline{\BX[\overline{\tx=t}]}}$, the definitions $\overline{D}$ must contain at least the definitions of all callbacks of the behaviours $\overline{\BX}$, which must be (fully) instantiated as indicated in $\overline{\BX[\overline{\tx=t}]}$. 

The first conditions is checked by the rule \Rule{WFModType-Arrow}, which tries to deduce the good formation of $T_{j+1}$ for all $j$ from the assumption that the names $N_i$ are available in the environment with their types $T_i$ for all $i\leq j$.
  ...
  \begin{mathpar}
    ...
    \inferrule[WFModType-Module]
      { \Sigma;\Gamma\vdash\overline{D}\\ 
        (\text{for }i=1..n\quad \Sigma;\Gamma\vdash{\overline{t:\star}}^i\\\BX_i:\Lambda(\overline{\tx:\star}^i).\{\overline{D}^i\}\in\Sigma)\\
        \forall i\neq j.
...
The second condition is checked by the rule \Rule{WFModType-Module}, by far the most complex one. This checks the good formation of the type of a module that implements the behaviours $\BX_i$ with instantiations $\overline{\tx=t}^i$. For that it checks that the definitions $\overline{D}$ are well-formed ($\Sigma;\Gamma\vdash\overline{D}$, using the rule \Rule{WFModType-DeclList}) and that all the $\overline{\,t\,}^i$ are well-formed ($\Sigma;\Gamma\vdash{\overline{t:\star}}^i$). Next it checks that for each behaviour $\BX_i$ implemented by the module, there is a definition in $\overline{D}^i$ in $\Sigma$ with the right parameters $\overline{\tx}^i$ (i.e., $\BX_i:\Lambda(\overline{\tx:\star}^i).\{\overline{D}^i\}\in\Sigma$). The rule then proceeds to a sanity check, which ensures that the implemented behaviours do not declare the same name (i.e., $\dom(\overline{D}^i)\#\dom(\overline{D}^j)$ for $i\neq j$) \beppe{\textbf{Not sure that the following is needed:} and that the parameters of the implemented behaviours do not have the same name (i.e., $\overline{x}^i\#\overline{x}^j$ for $i\neq j$)}. Once this sanity check is passed, the rule creates a new module type formed by all the declarations $\overline{D}^i$ of the implemented behaviours, where each type parameter $\tx$ is instantiated with the corresponding type $t$ given in the instantiation $\overline{\tx=t}^i$ of $\BX_i$ (i.e., $\{\overline{\overline{D}^i[\overline{\tx:=t}^i]}^{i=1..n}\}$). Finally, the rule checks that the module type $\{\overline{D}\}$ is a subtype of the module type formed by all the declarations of the implemented behaviours, which ensures that all the declarations required by the implemented behaviours are actually provided by the module with compatible types. Notice that the instatiantion is performed on the behaviour declarations $\overline{D}^i$ and not on the module type $\{\overline{D}\}$, since the module type $\{\overline{D}\}$ cannot refer to the parameters of the implemented behaviours.

Finally, the rule \Rule{WFModType-TArrow} is straightforward and just checks the good formation of the body of a parametric module type, under the assumption that the parameters are well-formed types.
...
Core types are always well-formed (provided that they are checked under well-formed environments) and the rules for type connectives and type constructors just check the good formation of their subterms, which is possible since pretypes are inductively defined.
A type name $\tx$ is well-formed if $\Gamma$ contains an association for it (rule \Rule{WFType-Name}). 
The behaviour $\BX[\overline{\tx=t}]$ is well-formed if $\Sigma$ contains the definition of $\BX$ with parameters $\overline{\tx}$ and these are instantiated by well-formed types (rule \Rule{WFType-Behaviour} also checks the good formation of the environments in case there were no instantiation).
...
The fact that there is no instanciation except for the last module has to be
...
P:\overline{D}$ and $\tx\in \dom(\overline{D})$. The rule \Rule{Path-Empty} for
