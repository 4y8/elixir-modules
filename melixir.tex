...
We used a minimal set of expressions to represent Elixir, since we want to focus on the module system. So our expressions are a simple functional language with records (in Elixir parlance ``maps'') and a type-case expression (in lieu of more general case expressions). The syntax is essentially the one by~\cite{} with two notable additions (in red): $M$, denoting the module (reached by the path) $M$, and $M.x$ denoting the function or constant $x$ exported by the module $M$.
...
\begin{figure}
  ...
  %  & | & $[\overline{\tx:\star}]\to T$ & do we want this syntax or remove it and have $\left\{\overline P;\overline{S};\overline{D}\right\}$?\\
    & | & $\left\{\textcolor{Cerulean}{\overline{P}};\overline{S};\overline{D}\right\}$ & the programmer could denote this type only via (\textsf{like }$M$)\\
    $t$ & ::= & $\kwint$ \\
    ...
    & | & $\mathbb{O}$ \\
    & | & $\textcolor{OrangeRed}{M.\tx}$ \\
    & | & $\textcolor{OrangeRed}{\mathcal{X}[\,\overline{\tx=t}\,]}$  & shall we move this into $T$? Consequence no $\kwtp x = \elix{Stack}$\\
    $D$ & ::= & $\kwmdl X : T$ \\
...
\end{minted}
Although the current Elixir syntax does not allow, in the system of Figure~\ref{fig:surfacesyntax} we can also define the module stack to be parametric in the type of
...
The types for expressions are essentially those of \cite{} extended with ...\textbf{TO BE CONTINUED} 
