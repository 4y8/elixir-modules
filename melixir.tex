...
The full syntax of the surface language is given in Figure \ref{fig:surfacesyntax}. Let us examine in details the definition of a module, as given in \eqref{eq:module}.

\begin{figure}
  \begin{tabular}{r c l}
    $\Sigma$ & ::= &$\kwdefmt X \kwdo \overline{P}\, \overline{D} \kwend$ \\
    $N$ & ::= & $x$ \\
    & | & $X$ \\
    $S$ & ::= & $\kwbvr \mathcal X$ \\
    $P$ &::= & $\kwprm x$ \\
    ...
    & | & {\color{red}$\mathcal{X}\left[\overline{x=t}\right]$} \\
    $D$ & ::= & $\kwmdl X : T$ \\
