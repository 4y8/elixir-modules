...
Although the letters $D$ and $\mathcal{D}$ look similar, there should not be confusion below.
When $D$ appears in a local environment or in a type, it is always a local declaration.
...
The wellformedness of declaration lists is an auxiliary judgement which is only used to define the
wellformedness of module environments.
...
We introduce two new notations:
...
We can now give inference rules for the wellformedness of declaration lists.
...
\[
    \begin{prooftree}
        ...
        \infer1 { P; \Sigma; \Gammatop; \Gammabot \vdash D, \overline{\mathcal{D}'} }
    \end{prooftree}
\]

Note that in the rule above, $P$ cannot be the empty path, because local declarations
can only appear inside a module.

\[
    \begin{prooftree}
        \hypo { \Pi; \Sigma; \Gammatop; \Gammabot \vdash X : \Moddecl }
        \hypo { \Pi; \Sigma; \Gammatop\left[\Pi \texttt{+=} X : \Moddecl\right]; \Gammabot \vdash \overline{\mathcal{D}'} }
        \infer2 { \Pi; \Sigma; \Gammatop; \Gammabot \vdash X : \Moddecl, \overline{\mathcal{D}'} }
    \end{prooftree}
\]

We give the wellformedness of $X : \Moddecl$ in a separate rule below only for legibility reasons.
By abuse of notation, we consider that $\epsilon.X$ denotes $X$. This avoids duplicating the rule.
\[
    \begin{prooftree}
...
\antonin{To check: is it a cyclic definition?}
...
\[
    \begin{prooftree}
        \hypo { \overline{\Sigma; \Gammatop; \Gammabot \vdash N : T_\downarrow} }
...
The hypothesis $\overline{\Sigma; \Gammatop; \Gammabot \vdash N : T_\downarrow}$ means that
$\Sigma; \Gammatop; \Gammabot \vdash \overline{N}^i : \overline{T_\downarrow}^i$ must hold for all $i$
without changing $\Gammabot$. It is not to be confused with the wellformedness judgement 
$\Sigma; \Gammatop \vdash \Gammabot, \overline{N : T_\downarrow}$.
