...
\end{lemma}
Thus if a behaviour, say \texttt{StackB}, has a parameter \texttt{elem}, then the encoding of \texttt{StackB[elem=integer()]} is the record type $\texttt{\%}\!\left\{\texttt{@n: :StackB},\texttt{elem}:\texttt{eq}(\texttt{integer()})\right\}$.


The encoding of the singleton type $\BH M$ of a fully instantiated module $M$ is similar to the one of behaviours, but instead of using a special field \texttt{@n} for the name, we use the special ``path'' field \texttt{@p} to encode the path of the module $M$. The encoding of $\BH{X.Y[\tx=t]}$ is then the record type $\texttt{\%}\!\left\{\texttt{@p}: \atom{X.Y},\tx:\texttt{eq}(t')\right\}$, where $t'$ is the encoding of $t$. If 
