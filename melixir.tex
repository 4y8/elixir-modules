...
\end{itemize}
Notice that we do not require $\Sigma$ and $\Gamma$ to be well-formed, that is, we do not require that they contain only well-formed (module) types. This will be required for typing, but for defining the encoding of well-formed types into core types we just need to check that paths point somewhere and that all parameters declared for a module or a behaviour are instantiated with well-formed simple types.
