...
\begin{figure}
  \begin{mathpar}
    \inferrule[Bind-DefModule]
       ...
       {\Sigma; \Gamma\vdash\kwdefm X\kwdo \overline{P}\overline{S}\overline{B}
        \kwend \overline{B_0}:X:\left(\overline{P:\star}\right)\rightarrow
        \left\{\overline{S};\overline{D}\right\} D_0}\\
    \inferrule[Bind-Type]
       {\Sigma;\Gamma\vdash t:\star\\
        \Sigma;\Gamma\vdash, x:[=t]\vdash\overline{B}:\overline{D}}
       {\Sigma;\Gamma\vdash\kwtp x = t,\overline{B}: \left(x:[=t],\overline{D}\right)}\\
    \inferrule[Bind-Opaque]
       {\Sigma;\Gamma\vdash t:\star\\
        \Sigma;\Gamma\vdash, x:[=t]\vdash\overline{B}:\overline{D}}
       {\Sigma;\Gamma\vdash\kwopq x = t,\overline{B}: \left(x:\star,\overline{D}\right)}\\
    \inferrule[Bind-Empty]
       {\\}{\Sigma;\Gamma\vdash\epsilon:\epsilon}\hfill
    \inferrule[Bind-Value]
       {\Sigma;\Gamma\vdash v:t\\
        \Sigma;\Gamma,x:t\vdash\overline{B}:\overline{D}}
       {\Sigma;\Gamma\vdash x=v,\overline{B}:\left(x:t,\overline{D}\right)}
  \end{mathpar}
