...
Finally, we encode a type names $\tx$ by the encoding it as $M.\tx$ where $M$ is the absolute path of the current module, instantiated with $[\tx=\tx]$. This is just a technical trick to be able to identify in a module $M$ the types $\tx$ and $M.\tx$ as being the same type.
...
The rules in Figures~\ref{wf-behenv} state that the empty behaviour environment is well-formed \textsc{[WFBehEnv-Empty]}; also, that if we assume that type names that are parameters of the behaviour \BX are well-formed types, we can deduce the good-formation of $\{\overline{D}\}$, then adding $\BX:\Lambda(\overline{\tx:\star}).\{\overline{D}\}$ to the current behaviour environment yields a well-formed environment \textsc{[WFBehEnv-Beh]}. Notice that no behaviour instantiation is given for $\{\overline{D}\}$ (i.e., we used  $\{\overline{D}\}$ rather than  $\{\overline{D}\}_{\overline{\BX[\overline{\tx=t}]}}$), since behaviours cannot implement other behaviours.
\begin{figure}[ht]
  \begin{mathpar}
...
\begin{figure}[ht]
  \begin{mathpar}
    ...
    \inferrule[WFValEnv-Concrete]{\Sigma;\Gamma\vdash t:\star}{\Sigma;\Gamma\vdash\Gamma,\tx:[= t]}\\
