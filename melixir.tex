...
$$\tau ::= \texttt{atom} \mid \texttt{mod} \mid \texttt{fun} \mid (\overline{\tau})\to\tau \mid \texttt{\%}\!\left\{\overline{\ell:\tau}\right\} \mid \tau\vee\tau \mid \neg\tau\mid\alpha\mid\mathbb{O}$$
The simple types $t$ are then defined on the top of the core types $\tau$ \emph{inductively} by the following grammar:
$$t ::= \tau \mid  (\overline{t})\to t \mid \texttt{\%}\!\left\{\overline{\ell:t}\right\} \mid M.\tx \mid \BH M \mid \BX[\overline{\tx=t}]$$
...
\begin{itemize}
\item core types
\item pretypes (not yet well formed)
